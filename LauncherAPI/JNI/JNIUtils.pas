...
function AnsiToWide(const AnsiString: UTF8String; CodePage: Word = CP_ACP): WideString;
var
  L: Integer;
begin
  if AnsiString = '' then
...
begin
  ...
  // Get the value
  {$IFDEF UNICODE}
  ReturnType := Copy(JSig, Pos(')', AnsiToWide(JSig)) + 1, Length(JSig));
  {$ELSE}
    ...
    case ReturnType[1] of
      ...
      'L':  // object
        if ReturnType = 'Ljava/lang/String;' then
          ...
          raise EJNIError.Create(
            {$IFDEF UNICODE}
            Format(NoObjectReturn, [Copy(MethodSig, 1, Pos('(', AnsiToWide(MethodSig)) - 1)]));
            {$ELSE}
...
begin
  ...
  if ReturnType[1] <> 'L' then
    raise EJNIError.Create(
      {$IFDEF UNICODE}
      Format(NoPrimitiveReturn, [Copy(MethodSig, 1, Pos('(', AnsiToWide(MethodSig)) - 1)]));
      {$ELSE}
