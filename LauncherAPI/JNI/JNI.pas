...
begin
  // JavaJNIStr->WideString
  Result := JNI_JStringToWideString(FEnv, Jstr);
  //Result := UTF8Decode(JStringToString(JStr));
end;
...
begin
  // WideString->UTF8Str->JavaJNIStr
  Result := JNI_WideStringToJString(FEnv, WStr);
  //Result := StringToJString(PAnsiChar(UTF8Encode(WStr)));
end;
...
begin
  ...
  if Chars = nil then
    ...
    Result := UTF8Decode(UTF8String(Chars));
    Env^.ReleaseStringUTFChars(Env, JStr, Chars);
  end;
}
var
...
//  sutf8: UTF8String;
//begin
//  sutf8 := UTF8Encode(WStr);
//  Result := Env^.NewStringUTF(Env, PAnsiChar(sutf8));
begin
   Result := Env^.NewString(Env, PJChar(PWideChar(WStr)), Length(WStr));
end;
